...
const endpoint = function(server, domainId, recordId) {
  ...
  const url = [server, 'domains', domainId, 'records', recordId];
...
if (isBrowser === true) {
  ...
  console.warn(
    'Ackee is not executing automatically because you are using it in an environment without a `window` object',
  );
}
